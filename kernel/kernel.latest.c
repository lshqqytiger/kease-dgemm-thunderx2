...
void micro_dxpy(
    ...
    for (uint64_t i = 0; i < m; ++i) {
        for (uint64_t j = 0; j < n; ++j) C[j] += _C[j];
        C += ldc;
        _C += NR;
    }
}
...
//    for (uint64_t pid = TOTAL_CORE - 1; pid < TOTAL_CORE; --pid) {
...
//        if (pid > 0) {
            ...
            err = pthread_attr_setaffinity_np(&attr, sizeof(cpu_set_t), &mask);
            if (err != 0)
                handle_error_en(err, "pthread_setaffinity_np");

            err = pthread_create(&tinfo[pid].thread_id, &attr, &middle_kernel, &tinfo[pid]);
            if (err != 0)
    ...
    double t[64];
//    for (uint64_t pid = 1; pid < TOTAL_CORE; ++pid) {
    for (uint64_t pid = 0; pid < TOTAL_CORE; ++pid) {
        void* res;
        int err = pthread_join(tinfo[pid].thread_id, &res);
        if (err != 0)
