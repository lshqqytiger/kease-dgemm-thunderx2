...
static void origin_micro_kernel (
    uint64_t kk,
    const double *restrict _A,
    const double *restrict _B,
    double *restrict C, uint64_t ldc,
...
//	origin
	...
	uint64_t mmc = (mm + MR - 1) / MR;
	uint64_t mmr = mm % MR;
	for (uint64_t mmi = mmc-1; mmi < mmc; --mmi) {
		uint64_t mmm = (mmi != mmc - 1 || mmr == 0) ? MR : mmr;
		for (uint64_t i = 0; i < mmm; ++i) {
			for (uint64_t j = kk-1; j < kk; --j) {
				_A[mmi * MR * kk + i + j * MR] = A[mmi * MR * lda + i * lda + j];
			}
		}
	}
	*/
...
#ifdef PLAY
    memset(_A, 0, sizeof(double) * (MB + MR) * KB);
    memset(_B, 0, sizeof(double) * KB * (NB + NR));
	...
	for (uint64_t ni = 0; ni < nc; ++ni) {
		...
		for (uint64_t ki = 0; ki < kc; ++ki) {
			...
			for (uint64_t mi = 0; mi < mc; ++mi) {
				...
				inner_kernel(mm, nn, kk, _A, _B, C, ldc);
			}
		}
	}
#else
